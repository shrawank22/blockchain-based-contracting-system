...
contract PartyContract{
    ...
    function createBid(address _bidderAddress, uint256 _tenderAddress, string memory _bidClause, uint256 _quotedAmount) public{
        ...
        require(tender.tenderStatus == TenderStatus.CLOSED, "Tender is not open for bids");
    ...
    modifier canViewBid(address _bidderAddress, address _issuerAddress ,uint256 _tenderId){
        require(msg.sender == _bidderAddress || tenders[_tenderId].issuerAddress == _issuerAddress, "Not authorized to view bid details");
        _;
    }

    // For getting bid details like message, amount, address of bidders 
    function getBidDetails(address _bidderAddress, address _issuerAddress ,uint256 _tenderId, uint256 _bidId) public view canViewBid(_bidderAddress, _issuerAddress, _tenderId) returns (Bid memory) {
    ...
    function selectWinnerBid(address _partyAddress, uint256 _tenderId ) public view isTenderOwner(_partyAddress, _tenderId) isValidTender(_tenderId) returns (Bid[] memory) {
